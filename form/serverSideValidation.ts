...
export default function validateForm(requiredFields: FormField[]) {
    return requiredFields.flatMap(validateInput);
}
...
function validateInput(input: FormField) {
...
function hasInvalidLength(input: FormField) {
    const { value, minLength, maxLength } = input;

    const tooShort = minLength > 0 && value.length < minLength;
    const tooLong = maxLength > 0 && value.length > maxLength;

    return tooShort || tooLong;
}

function matchesPattern(input: FormField) {
